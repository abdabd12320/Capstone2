...
public class BrandController {
    ...
    @PostMapping("/add")
    public ResponseEntity addBrand(@Valid@RequestBody Brand brand, Errors errors)
    {
        if(errors.hasErrors())
        {
    ...
    public ResponseEntity updateBrand(@PathVariable Integer id,@Valid@RequestBody Brand brand,Errors errors)
    {
        if(errors.hasErrors())
        {
            String m = errors.getFieldError().getDefaultMessage();
