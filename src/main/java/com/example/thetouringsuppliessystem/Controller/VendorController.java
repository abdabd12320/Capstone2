...
public class VendorController {
    ...
    @PostMapping("/add")
    public ResponseEntity addVendor(@Valid@RequestBody Vendor vendor, Errors errors)
    {
        if(errors.hasErrors())
        {
    ...
    public ResponseEntity updateVendor(@PathVariable Integer id,@Valid@RequestBody Vendor vendor, Errors errors)
    {
        if(errors.hasErrors())
        {
            String m = errors.getFieldError().getDefaultMessage();
