...
public class VendorStockController {
    ...
    @PostMapping("/add")
    public ResponseEntity addVendorStock(@Valid@RequestBody VendorStock vendorStock, Errors errors)
    {
        if(errors.hasErrors())
        {
    ...
    public ResponseEntity updateVendorStock(@PathVariable Integer id,@Valid@RequestBody VendorStock vendorStock, Errors errors)
    {
        if(errors.hasErrors())
        {
            String m = errors.getFieldError().getDefaultMessage();
